...
- 부모테이블과 자식테이블 사이에 삭제 또는 수정할 때 규칙이 있다.
  ...
  - RULE
    ...
    <BR>③<span style ="background-color:#f1f8ff"> no action</span> : 참조되는 테이블에서 데이터를 삭제하거나 수정해도, 참조하는 테이블의 데이터는 변경되지 않는다.
    <BR>④ <span style ="background-color:#f1f8ff">set default</span> : 참조되는 테이블에서 데이터를 삭제하거나 수정하면, 참조하는 테이블의 데이터는 필드의 기본값으로 설정된다.
    <BR>⑤ <span style ="background-color:#f1f8ff">restrict</span> :  참조하는 테이블에 데이터가 남아 있으면, 참조되는 테이블의 데이터를 삭제하거나 수정할 수 없다.
